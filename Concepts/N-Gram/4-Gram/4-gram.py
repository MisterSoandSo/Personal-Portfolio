...
quadTable = {}  # key: (int,int,int) → value: {next_token: count}
#=====================================================================
...
def save_checkpoint(biTable, triTable,quadTable, k, filename="checkpoint.pkl"):
    with open(filename, 'wb') as f:
        pickle.dump({'bi': biTable, 'tri': triTable, 'quad': quadTable, 'last_k': k}, f)

def load_checkpoint(filename="checkpoint.pkl"):
    try:
        with open(filename, 'rb') as f:
            data = pickle.load(f)
            return data['bi'], data['tri'], data['quad'], data['last_k']
...
def processToken(k):
    ...
    # Quadgram 
    key = (t1, t2,t3)
    if key in quadTable and t4 in quadTable[key]:
        quadTable[key][t4] += 1 
    else:
        if key not in quadTable:
            quadTable[key] = {}
        quadTable[key][t4] = 1

biTable, triTable, quadTable, start_k = load_checkpoint()

for k in range(start_k, Dataset_len - 3):
    processToken(k)
    '''if k % 10000 == 0:
        save_checkpoint(biTable, triTable, quadTable, k)
save_checkpoint(biTable, triTable, quadTable, Dataset_len - 3)
'''
...
while True:
    ...
    token1, token2, token3 = ids[-3], ids[-2], ids[-1]
    out = [tokenizer.id_to_token(token1), tokenizer.id_to_token(token2), tokenizer.id_to_token(token3)]
    print(ids)
    
    loopcnt = 0
    while loopcnt < 10:
        key4 = (token1, token2, token3)
        if not quadTable.get(key4, {}):
            print("[DEBUG] No quadgram match. Ending generation.")
            break
        p4 = normalize(quadTable.get(key4, {}))
        key3 = (token1, token2)
        ...
        candidates = set(p4.keys()) |set(p3.keys()) | set(p2.keys())
        probs = {token: lambda4 * p4.get(token, 0) + lambda3 * p3.get(token, 0) + lambda2 * p2.get(token, 0) for token in candidates}

        # Filter out recently used tokens (basic repetition ban)
        for recent in recent_keys:
            if recent in probs:
                probs[recent] *= 0.01  # Penalize heavily
        ...
        recent_keys.append(next_token)
        if len(recent_keys) > 10:  # Keep only recent N tokens
            recent_keys.pop(0)

        print(f"\n[DEBUG] Loop {loopcnt}")
        print(f"  key4 (quad-history): {key4} -> p4 keys: { list(p4.keys())}")
        print(f"  key3 (tri-history): {key3} -> p3 keys: {list(p3.keys())}")
        print(f"  key2 (bi-history): {key2} -> p2 keys: {list(p2.keys())[:20]}")
        print(f"  Recent tokens (penalized): {recent_keys}")
        print(f"  Next token selected: {next_token} ({tokenizer.id_to_token(next_token)})")
        token1, token2,token3 = token2,token3, next_token
        loopcnt += 1
